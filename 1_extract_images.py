...
#python2 1_extract_images.py --data_dir ${OUTPUT_DIR} --is_sim
#rosbag play -r ${RATE} --clock ${BAG_PATH}

class BagExtract():
    def __init__(self, data_dir, process_rect_images, process_joints, left_only, is_sim):
        ...
        if self.is_sim:
            self.disparity_dir = os.path.join(data_dir, 'disparities')
    ...
    def make_dirs(self):
        ...
        if self.is_sim:
            if not os.path.exists(self.disparity_dir):
                os.mkdir(self.disparity_dir)
    ...
    def process_images(self, left_image_raw_ros, right_image_raw_ros, 
        ...
        if self.process_rect_images:
            ...
            if (not self.left_only) and (not self.is_sim):
                right_image_rect = self.bridge.imgmsg_to_cv2(right_image_rect_ros, desired_encoding='bgr8')

        if self.is_sim:
            disparity_image = self.bridge.imgmsg_to_cv2(disparity_image_ros, desired_encoding='passthrough')
        ...
        if self.is_sim:
            disparity_image_path = os.path.join(self.disparity_dir, msg_string + '.npy')
            disparity_array = np.array(disparity_image, dtype=np.float32)
            np.save(disparity_image_path, disparity_array)
    ...
    def process_disparity(self, left_camera_info_ros, right_camera_info_ros,
                            left_image_rect_ros,
                            disparity_image_ros):
        self.process_images(None, None, left_camera_info_ros, right_camera_info_ros,
                            left_image_rect_ros, None, disparity_image_ros)
    
    def get_process_image_callback(self):
        if self.is_sim:
            return self.process_disparity
...
def start(data_dir, process_rect_images, process_joints, left_only, is_sim, 
          queue_size=10, slop=0.3):
    bag_extractor = BagExtract(data_dir, process_rect_images, process_joints, left_only, is_sim)
    ...
    if process_rect_images:
        ...
        if (not left_only) and (not is_sim):
            right_image_rect_topic = "theia/right/image_rect_color"

    if is_sim:
        disparity_topic = "disparity_image"
    ...
    if is_sim:
        disparity_image_sub = message_filters.Subscriber(disparity_topic, Image)
    ...
    if is_sim:
        async_topics.append(disparity_image_sub)
...
if __name__ == "__main__":
    ...
    start(data_dir, process_rect_images, process_joints, left_only, is_sim)
